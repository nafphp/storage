...
final class LocalAdapter implements StorageAdapterInterface
{
    ...
    public function __construct(
        ...
        $absolute = str_starts_with($root, '/')
            || (DIRECTORY_SEPARATOR === '\\' && preg_match('~^[A-Za-z]:[/\\\\]~', $root));
        if (!$absolute || str_contains($root, "\0") || str_contains($root, '://')) {
            throw new StorageException('The local storage root must be an absolute filesystem directory.');
        }
        if ($fileMode < 0 || $fileMode > 0777 || $directoryMode < 0 || $directoryMode > 0777) {
    ...
    public function move(string $source, string $destination): void
    {
        ...
        $this->perform(function () use ($source, $destination): void {
            $from = $this->resolve($source);
            $this->requireFile($from, $source);
            if ($source === $destination) {
                return;
            }

            $to = $this->resolve($destination, true);
            if (!rename($from, $to)) {
    ...
    private function resolve(string $path, bool $writing = false): string
    {
        ...
        foreach ($segments as $index => $segment) {
            ...
            if (is_dir($fullPath) && (!is_readable($fullPath)
                || (DIRECTORY_SEPARATOR === '/' && !is_executable($fullPath)))
            ) {
            ...
            } elseif (file_exists($fullPath) && !is_file($fullPath) && ($writing || !is_dir($fullPath))) {
    ...
    private static function writeContents(mixed $output, string $contents): void
    {
        ...
        for ($offset = 0; $offset < $length; $offset += $written) {
            $written = fwrite($output, substr($contents, $offset, 8192));
     ...
     */
    private function perform(callable $operation, string $message, string $exceptionClass = StorageException::class): mixed
    {
